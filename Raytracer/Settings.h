...
const int NUM_TRACE_CALLS = 6; // number of bounces to trace each ray
const float AREA_LIGHT_SAMPLES = 30.0f; // number of samples to take from an area light for a point on a surface
...
const int SAMPLES = 1; // number of samples per pixel
...
const int NUM_PHOTONS = 100000; // global photons per light source
...
const float CAUSTIC_MAX_DISTANCE = 1.0f; // max distance to look for photons in irradiance estimate
...
const float APERTURE_RADIUS = 0.18f; // radius of camera aperture
const float FOCUS_DISTANCE = 10.44f; // plane in focus (distance from camera)
#endif